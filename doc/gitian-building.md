...
*Setup instructions for a gitian build of EazyPayZA using a Debian VM or physical system.*
...
- [Building EazyPayZA](#building-eazypayza)
...
Clone the git repositories for eazypayza and gitian and then checkout the eazypayza version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the eazypayza repository
...
To build EazyPayZA (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the eazypayza repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/eazypayza/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/eazypayza/eazypayza
...
For example:
```bash
URL=https://github.com/konez2k/eazypayza.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/eazypayza-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/eazypayza-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/eazypayza-build.assert
```
...
[eazypayza/gitian.sigs](https://github.com/eazypayza/gitian.sigs/) repository, or if that's not possible to create a pull
request.
